...
/**
 * Clase que realiza la conversión de Expresión Regular a Autómata Finito.
 * Contiene los métodos para tratar uniones, concatenaciones y cerraduras de
 * Kleene.
 * 
 */
public class Convertidor {
  ...
  public Convertidor(String expresionRegular) {
    ...
    this.automataFinito = new AFD(q0, q1);
  ...
  public void convertir() {
    transformarEstadosInicalFinal(q0, er, q1);
  }
  ...
  /**
   * Separar las concatenaciones de la expresión y crear las transiciones entre
   * ellas
   * 
  ...
  private void transformarConcatenacion(Estado estadoOrigen, Expresion expresion, Estado estadoDestino) {
    // System.out.println("\nPaso " + numeroPaso + ". Separar concatenacion en " +
    // expresion.getExpresionRegular() + ": ");
    numeroPaso++;
    ...
    Estado estadoIntermedio = automataFinito.crearEstado(); // Para las transiciones entre los estados de origen y
                                                            // destino
    ...
    // Buscar si las subexpresiones se pueden transformar, esto se hace al final de
    // cada transformación
  ...
  private void transformarUnion(Estado estadoOrigen, Expresion expresion, Estado estadoDestino) {
    // System.out.println("\nPaso " + numeroPaso + ". Separar uniones en " +
    // expresion.getExpresionRegular() + ": ");
    numeroPaso++;
  ...
  private void transformarCerraduraKleene(Estado estadoOrigen, Expresion expresion, Estado estadoDestino) {
    // System.out.println("\nPaso " + numeroPaso + ". Separar cerradura de Kleene en
    // " + expresion.getExpresionRegular() + ": ");
    numeroPaso++;
    ...
    // Crear la transición del estado intermedio así mismo con la expresión sin
    // cerradura de Kleene
  ...
  /**
   * Identificar si la expresión se trata de una unión, concatenación o cerradura
   * de Kleene.
  ...
  private void identificarSiguienteTransformacion(Estado estadoOrigen, Expresion expresion, Estado estadoDestino) {
    if (expresion.esExpresionMinima())
      return;
    if (expresion.esUnion())
      transformarUnion(estadoOrigen, expresion, estadoDestino);
    else if (expresion.esConcatenacion())
      transformarConcatenacion(estadoOrigen, expresion, estadoDestino);
    else if (expresion.esCerraduraKleene())
      transformarCerraduraKleene(estadoOrigen, expresion, estadoDestino);
  }
  ...
  public void imprimirAFD() {
    ...
    try {
      automataFinito.imprimirArchivo("salida.afd");
